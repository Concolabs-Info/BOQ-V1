...
import type { DemoStatus, Opening, RoofZone, Wall, Zone } from "./types";
...
export function wallLengthM(w:Wall){ return distance(w.start,w.end)*scaleForViewport(w.viewportId); }
export function wallGrossAreaM2(w:Wall){ return wallLengthM(w)*w.heightM; }
...
export function wallNetAreaM2(w:Wall){ return Math.max(0,wallGrossAreaM2(w)-wallOpeningDeductM2(w)); }
...
export function buildReviewStateLegacy(projectId:string, floorId:string|null, category:string):ReviewState{
  ...
  for(const column of structure.columns){const family=structure.columnFamilies.find(x=>x.id===column.familyId)!;items.push({id:`REV-${column.id}`,project_id:projectId,floor_id:column.floorId,entity_type:"column",entity_id:column.id,display_number:family.mark,title:`${family.mark} · ${family.description}`,data:{floor:floorName(column.floorId),type_code:family.mark,description:family.description,shape:family.shape,width_mm:family.widthMm,depth_mm:family.depthMm,diameter_mm:family.diameterMm,height_m:column.heightM,source:family.source,value_sources:{geometry:"model",family:"specification"}},status:statusToReview(column.status),critical:false,is_stale:false,source_version:1,review_version:column.status==="confirmed"?2:1})}
  for(const beam of structure.beams){const family=structure.beamFamilies.find(x=>x.id===beam.familyId)!,length=distance(beam.start,beam.end)*scaleForViewport(beam.viewportId);items.push({id:`REV-${beam.id}`,project_id:projectId,floor_id:beam.floorId,entity_type:"beam",entity_id:beam.id,display_number:family.mark,title:`${family.mark} · ${family.description}`,data:{floor:floorName(beam.floorId),type_code:family.mark,description:family.description,kind:beam.kind,width_mm:family.widthMm,depth_mm:family.depthMm,drop_mm:beam.dropMm,length_m:length,source:family.source,value_sources:{geometry:"model",family:"specification"}},status:statusToReview(beam.status),critical:false,is_stale:false,source_version:1,review_version:beam.status==="confirmed"?2:1})}
  ...
  for(const finish of st.wallFinishFamilies){
    for(const fid of [...new Set(st.walls.filter(w=>w.side1Finish===finish.id||w.side2Finish===finish.id).map(w=>w.floorId))]){
      const related=st.walls.filter(w=>w.floorId===fid&&(w.side1Finish===finish.id||w.side2Finish===finish.id));
...
export function buildReviewState(projectId:string, floorId:string|null, category:string):ReviewState{
  ...
  const special=useSpecialStore.getState();
  const items:ReviewItem[]=[];
  const nrm2Items=(type:ReviewItem["entity_type"])=>type==="column"||type==="beam"||type==="slab"?["NRM2 11 — In-situ concrete","NRM2 11 — Formwork"]:type==="stair"?["NRM2 25 — Stairs, walkways and balustrades (information required)"]:type==="wall"?["NRM2 13 — Masonry"]:type==="door"?["NRM2 24 — Doors, shutters and hatches"]:type==="window"?["NRM2 23 — Windows, screens and lights"]:type==="floor_finish"?["NRM2 28 — Floor finishes","NRM2 28.1 — Screeds where specified","NRM2 28.14 — Skirting where specified"]:type==="wall_finish"||type==="ceiling_zone"?["NRM2 28 — Floor, wall and ceiling finishes"]:type.startsWith("roof_")?["NRM2 19 — Waterproofing and roof work"]:[];
  ...
  structure.columns.forEach(item=>{const f=structure.columnFamilies.find(x=>x.id===item.familyId);if(f)add("column",item.id,item.floorId,f.mark,`${f.mark} · ${f.description}`,{floor:floorName(item.floorId),type_code:f.mark,description:f.description,shape:f.shape,width_mm:f.widthMm,depth_mm:f.depthMm,diameter_mm:f.diameterMm,height_m:item.heightM,source:f.source},item.status)});
  structure.beams.forEach(item=>{const f=structure.beamFamilies.find(x=>x.id===item.familyId);if(f)add("beam",item.id,item.floorId,f.mark,`${f.mark} · ${f.description}`,{floor:floorName(item.floorId),type_code:f.mark,description:f.description,kind:item.kind,width_mm:f.widthMm,depth_mm:f.depthMm,drop_mm:item.dropMm,length_m:distance(item.start,item.end)*scaleForViewport(item.viewportId),source:f.source},item.status)});
  ...
  special.flights.filter(item=>!item.evidenceOnly).forEach(item=>{const f=special.flightFamilies.find(x=>x.id===item.familyId);if(f)add("stair",item.id,item.floorId,f.mark,`${f.mark} · ${f.description}`,{floor:floorName(item.floorId),description:f.description,plan_area_m2:zoneAreaM2(item.points,item.voids,scaleForViewport(item.viewportId)),width_mm:f.widthMm,waist_mm:f.waistMm||null,riser_mm:f.riserMm||null,tread_mm:f.treadMm||null,quantity_status:f.waistMm>0?"measurable":"information_required",source:f.source},item.status,true)});
  ...
  st.wallFinishFamilies.forEach(f=>{const floorIds=[...new Set(st.walls.filter(w=>w.side1Finish===f.id||w.side2Finish===f.id).map(w=>w.floorId))];floorIds.forEach(fid=>{const walls=st.walls.filter(w=>w.floorId===fid&&(w.side1Finish===f.id||w.side2Finish===f.id));if(!walls.length)return;const area=walls.reduce((sum,w)=>sum+(w.side1Finish===f.id?wallNetAreaM2(w):0)+(w.side2Finish===f.id?wallNetAreaM2(w):0),0);const status:DemoStatus=walls.every(w=>w.status==="confirmed")?"confirmed":"ready";add("wall_finish",`WF-${f.id}-${fid}`,fid,f.mark,`${f.mark} · ${f.description}`,{floor:floorName(fid),finish_code:f.mark,wall_finish:f.description,material:f.material,thickness_mm:f.thicknessMm,area_m2:area,source:f.source},status)})});
...
export function buildBoqRowsLegacy():BoqRow[]{
  ...
  for(const f of structure.columnFamilies){const items=structure.columns.filter(item=>item.familyId===f.id);if(!items.length)continue;const floorIds=[...new Set(items.map(item=>item.floorId))];const quantity=items.reduce((sum,item)=>{const width=(f.shape==="Circular"?(f.diameterMm||f.widthMm):f.widthMm)/1000,depth=(f.shape==="Circular"?(f.diameterMm||f.depthMm):f.depthMm)/1000,area=f.shape==="Circular"?Math.PI*(width/2)**2:width*depth;return sum+area*item.heightM*floorFactor(item.floorId)},0);rows.push(row(`BOQ-COLUMN-${f.id}`,"Concrete structure — Columns",f.mark,`PRELIMINARY PARTIAL OVERLAY — ${f.description}; ${f.shape==="Circular"?`${f.diameterMm||f.widthMm} mm diameter`:`${f.widthMm} × ${f.depthMm} mm`}`,quantity,"m³",null,"column",floorIds,items.map(item=>item.id)))}
  for(const f of structure.beamFamilies){const items=structure.beams.filter(item=>item.familyId===f.id);if(!items.length)continue;const floorIds=[...new Set(items.map(item=>item.floorId))];let quantity=0;for(const floorId of floorIds)for(const kind of (["Downstand","Through"] as const)){const group=items.filter(item=>item.floorId===floorId&&item.kind===kind);if(!group.length)continue;const calculated=group.reduce((sum,item)=>sum+distance(item.start,item.end)*scaleForViewport(item.viewportId)*(f.widthMm/1000)*((item.kind==="Downstand"?Math.max(item.dropMm,1):f.depthMm)/1000),0)*floorFactor(floorId);quantity+=structure.workbookOverrides[`beams:${f.id}:${floorId}:${kind}`]??calculated}rows.push(row(`BOQ-BEAM-${f.id}`,"Concrete structure — Beams",f.mark,`PRELIMINARY PARTIAL OVERLAY — ${f.description}; ${f.widthMm} × ${f.depthMm} mm`,quantity,"m³",null,"beam",floorIds,items.map(item=>item.id)))}
...
export function buildBoqRows():BoqRow[]{
  ...
  structure.columnFamilies.forEach(f=>{const records=structure.columns.filter(x=>x.familyId===f.id&&x.status==="confirmed");if(!records.length)return;const quantity=records.reduce((sum,item)=>{const diameter=(f.diameterMm||f.widthMm)/1000;const area=f.shape==="Circular"?Math.PI*(diameter/2)**2:(f.widthMm/1000)*(f.depthMm/1000);return sum+area*item.heightM*floorFactor(item.floorId)},0);add(`BOQ-COLUMN-${f.id}`,"Concrete structure — Columns",f.mark,`${f.description}; ${f.shape==="Circular"?`${f.diameterMm||f.widthMm} mm diameter`:`${f.widthMm} × ${f.depthMm} mm`}`,quantity,"m³","column",[...new Set(records.map(x=>x.floorId))],records.map(x=>x.id))});
  structure.beamFamilies.forEach(f=>{const records=structure.beams.filter(x=>x.familyId===f.id&&x.status==="confirmed");if(!records.length)return;const quantity=records.reduce((sum,item)=>sum+distance(item.start,item.end)*scaleForViewport(item.viewportId)*(f.widthMm/1000)*((item.kind==="Downstand"?Math.max(item.dropMm,1):f.depthMm)/1000)*floorFactor(item.floorId),0);add(`BOQ-BEAM-${f.id}`,"Concrete structure — Beams",f.mark,`${f.description}; ${f.widthMm} × ${f.depthMm} mm`,quantity,"m³","beam",[...new Set(records.map(x=>x.floorId))],records.map(x=>x.id))});
  ...
  st.wallFamilies.forEach(f=>{const records=st.walls.filter(x=>x.familyId===f.id&&x.status==="confirmed");if(!records.length)return;const quantity=records.reduce((sum,item)=>sum+wallNetAreaM2(item)*floorFactor(item.floorId),0);add(`BOQ-WALL-${f.id}`,"Masonry",f.mark,`${f.description}; ${f.thicknessMm} mm; net of opening deductions`,quantity,"m²","wall",[...new Set(records.map(x=>x.floorId))],records.map(x=>x.id))});
  ...
  st.wallFinishFamilies.forEach(f=>{const records=st.walls.filter(x=>x.status==="confirmed"&&(x.side1Finish===f.id||x.side2Finish===f.id));if(!records.length)return;const quantity=records.reduce((sum,item)=>sum+((item.side1Finish===f.id?wallNetAreaM2(item):0)+(item.side2Finish===f.id?wallNetAreaM2(item):0))*floorFactor(item.floorId),0);add(`BOQ-WALL-FINISH-${f.id}`,"Wall finishes",f.mark,`${f.description}; ${f.material}`,quantity,"m²","wall_finish",[...new Set(records.map(x=>x.floorId))],records.map(x=>x.id))});
  ...
  structure.columnFamilies.forEach(f=>{const records=structure.columns.filter(x=>x.familyId===f.id&&x.status==="confirmed");if(!records.length)return;const perimeter=f.shape==="Circular"?Math.PI*((f.diameterMm||f.widthMm)/1000):2*((f.widthMm+f.depthMm)/1000);add(`BOQ-FORM-COLUMN-${f.id}`,"In-situ concrete — Formwork",`11.2-${f.mark}`,`Formwork to sides of ${f.description}`,records.reduce((sum,item)=>sum+perimeter*item.heightM*floorFactor(item.floorId),0),"m²","formwork",[...new Set(records.map(x=>x.floorId))],records.map(x=>x.id))});
  structure.beamFamilies.forEach(f=>{const records=structure.beams.filter(x=>x.familyId===f.id&&x.status==="confirmed");if(!records.length)return;const quantity=records.reduce((sum,item)=>{const length=distance(item.start,item.end)*scaleForViewport(item.viewportId);const depth=(item.kind==="Downstand"?Math.max(item.dropMm,1):f.depthMm)/1000;return sum+length*(2*depth+f.widthMm/1000)*floorFactor(item.floorId)},0);add(`BOQ-FORM-BEAM-${f.id}`,"In-situ concrete — Formwork",`11.2-${f.mark}`,`Formwork to sides and soffits of ${f.description}`,quantity,"m²","formwork",[...new Set(records.map(x=>x.floorId))],records.map(x=>x.id))});
  ...
  const classified=rows.map(item=>{
    ...
    if(item.entity_type==="wall"){subcategory_code="NRM2 13";subcategory_name="Masonry"}
...
export function buildBoqState(projectId:string,floorId:string|null):BoqState{
  ...
  const summary={rows:rows.length,ready:rows.filter(r=>r.status==="ready").length,needs_review:rows.filter(r=>r.status==="needs_review").length,manual:rows.filter(r=>r.manual).length,doors:rows.filter(r=>r.entity_type==="door").length,windows:rows.filter(r=>r.entity_type==="window").length,walls:rows.filter(r=>r.entity_type==="wall").length,wall_finishes:rows.filter(r=>r.entity_type==="wall_finish").length,floor_works:0,ceilings:rows.filter(r=>r.entity_type==="ceiling").length,roofs:rows.filter(r=>r.entity_type==="roof").length,floors:rows.filter(r=>r.entity_type==="floor_finish").length,subtotal:rows.reduce((s,r)=>s+Number(r.amount||0),0)};
