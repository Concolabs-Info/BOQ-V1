...
import type { FloorCropSaveInput, FloorCropSaveResult, FloorPlanPagesResult, FloorPlansState, FloorSourceUploadResult } from "./types";
...
export function getFloorPlanDocumentPages(
  projectId: string,
  documentId: string,
  offset = 0,
  limit = 24
): Promise<FloorPlanPagesResult> {
  const query = new URLSearchParams({ offset: String(offset), limit: String(limit) });
  return requestJson<FloorPlanPagesResult>(`${basePath(projectId)}/documents/${documentId}/pages?${query}`);
}
