...
export type FloorAnalysisQuality = "easy" | "medium" | "expert" | "maximum";

export function analyzeRooms(projectId: string, floorId: string, quality: FloorAnalysisQuality = "medium") {
  return requestJson(`${base(projectId)}/floors/${floorId}/analyze`, {
    method: "POST",
    body: JSON.stringify({ method: "ai", quality }),
  });
}

export function importFloorJson(projectId: string, floorId: string, result: Record<string, unknown>, quality: FloorAnalysisQuality = "medium") {
  return requestJson(`${base(projectId)}/floors/${floorId}/import-json`, {
    method: "POST",
    body: JSON.stringify({ result, quality }),
  });
}

export function downloadFloorCrop(projectId: string, floorId: string, floorName = "floor-crop") {
  return downloadApiFile(
    `${base(projectId)}/floors/${floorId}/crop`,
    `${floorName.replace(/[^a-z0-9_-]+/gi, "-") || "floor-crop"}.png`,
  );
}
