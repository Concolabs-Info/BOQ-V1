...
export function drawingSize(sheet?: { page: number }) {
  ...
  return sheet && sheet.page >= 23
    ? { width: 2573, height: 1820 }
    : { width: 1820, height: 2573 };
}
...
export function ViewportEditorOverlay({
  ...
  const viewport = useDemoStore((state) =>
    state.viewports.find((item) => item.id === viewportId),
  );
  const sheet = useDemoStore((state) =>
    state.sheets.find((item) => item.id === viewport?.sheetId),
  );
  const updateViewport = useDemoStore(
