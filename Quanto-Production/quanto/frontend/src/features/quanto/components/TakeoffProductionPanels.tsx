...
export function useTakeoffRows(element: string): ItemRow[] {
  ...
  return useMemo(() => {
    ...
    if (element === "floor" || element === "ceiling") {
      ...
      return zones.map((item) => { const family = families.find((f) => f.id === item.familyId); return { id:item.id, title:item.room || item.id, subtitle:family ? `${family.mark} · ${family.description}` : item.id, viewportId:item.viewportId, familyId:item.familyId, status:item.status, quantity:`${zoneNetAreaM2(item).toFixed(2)} m²` }; });
    }
...
export function HistoryPanel({ element }:{ element:string }) {
  ...
  return <div className="space-y-3 p-4"><Head title="Change tracking" sub="Current production state and reversible session edits."/><div className="rounded-xl border border-slate-200 p-3"><div className="flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${statusDot(row.status)}`}/><p className="text-xs font-bold text-slate-800">{statusLabel(row.status)}</p></div><p className="mt-2 text-[11px] leading-5 text-slate-500">Item {row.id} is using the current editable project geometry. Changes are saved through the existing production update path.</p></div><div className="rounded-xl border border-slate-200 p-3"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Undo stack</p><p className="mt-1 text-sm font-bold text-slate-800">{st.geometryUndo.length} reversible change{st.geometryUndo.length===1?"":"s"}</p><p className="mt-1 text-[11px] text-slate-500">Use Undo or Ctrl+Z to reverse the latest geometry edit.</p></div></div>;
}
export function TakeoffStatusBar({ element,viewportName,scale,snap,ortho }:{ element:string; viewportName:string; scale:number; snap:boolean; ortho:boolean }) {
  ...
  return <div className="flex h-8 items-center justify-between border-t border-slate-200 bg-white px-3 text-[10px] font-semibold text-slate-500"><div className="flex min-w-0 items-center gap-3"><span className="max-w-[220px] truncate">{viewportName}</span><span>Scale {scale>0?`1:${Math.max(1,Math.round(1/scale/3.7795275591))}`:"—"}</span><span>Snap {snap?"ON":"OFF"}</span><span>Ortho {ortho?"ON":"OFF"}</span></div><div className="flex items-center gap-3"><span>{review?`${review} need review`:"Review clear"}</span><span>{id?`Selected ${id}`:`${rows.length} items`}</span><span className="text-emerald-600">Saved ✓</span></div></div>;
}
