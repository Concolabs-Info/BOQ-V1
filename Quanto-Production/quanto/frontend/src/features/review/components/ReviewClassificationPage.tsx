...
const categoryOptions: Array<{ value: ElementCategory; label: string }> = [
  ...
  { value: "floor_finish", label: "Floor finishes" },
  { value: "wall_finish", label: "Wall finishes" },
...
function groupCode(item: ReviewItem): string {
  if (item.entity_type === "column" || item.entity_type === "beam" || item.entity_type === "slab" || item.entity_type === "stair") return cleanCode(item.data.type_code) || cleanCode(item.display_number) || "UNCLASSIFIED";
  ...
  if (item.entity_type === "wall_finish") {
    return cleanCode(item.data.finish_code) || cleanCode(item.data.wall_finish) || cleanCode(item.data.material_category) || "UNCLASSIFIED";
  }
  if (item.entity_type === "ceiling_zone") return cleanCode(item.data.definition_code) || cleanCode(item.data.ceiling_system) || "UNCLASSIFIED";
  if (item.entity_type === "roof_plane") return cleanCode(item.data.definition_code) || cleanCode(item.data.roof_system) || cleanCode(item.data.roof_type) || "UNCLASSIFIED";
...
function groupLabel(item: ReviewItem, code: string): string {
  if (item.entity_type === "column" || item.entity_type === "beam" || item.entity_type === "slab" || item.entity_type === "stair") return text(item.data.description) || `${code} ${item.entity_type}`;
  ...
  if (item.entity_type === "wall_finish") return text(item.data.wall_finish) || text(item.data.material) || "Wall finish not assigned";
  if (item.entity_type === "ceiling_zone") return text(item.data.ceiling_system) || text(item.data.material) || "Ceiling system not assigned";
  if (item.entity_type === "roof_plane") return text(item.data.roof_system) || text(item.data.material) || "Roof system not assigned";
...
function itemMeasure(item: ReviewItem): string {
  if (item.entity_type === "column") return `${text(item.data.width_mm)} × ${text(item.data.depth_mm)} mm · ${text(item.data.height_m)} m high`;
  if (item.entity_type === "beam") return `${number(item.data.length_m)?.toFixed(2) || "—"} m · ${text(item.data.width_mm)} × ${text(item.data.depth_mm)} mm`;
  if (item.entity_type === "slab") return `${number(item.data.net_area_m2)?.toFixed(2) || "—"} m² · ${text(item.data.thickness_mm)} mm`;
  ...
  if (item.entity_type === "ceiling_zone") return number(item.data.net_area_m2) ? `${number(item.data.net_area_m2)!.toFixed(2)} m²` : "Area needs review";
  if (item.entity_type === "roof_plane") return number(item.data.net_area_m2) ? `${number(item.data.net_area_m2)!.toFixed(2)} m²` : "Area needs review";
    ...
    ...new Set(
      items
        .map((item) => {
          ...
          if (item.entity_type === "column" || item.entity_type === "beam" || item.entity_type === "slab") return text(item.data.description) || "Reinforced concrete";
          if (item.entity_type === "wall") {
            return text(item.data.wall_type || item.data.side_1_finish || item.data.side_2_finish);
          }
          if (item.entity_type === "wall_finish") return text(item.data.wall_finish || item.data.material);
          if (item.entity_type === "ceiling_zone") return text(item.data.ceiling_system || item.data.material || item.data.finish);
          if (item.entity_type === "roof_plane") return text(item.data.roof_system || item.data.material || item.data.roof_type);
...
function areaTotal(items: ReviewItem[]): number | null {
  if (!items.length || !["wall", "floor", "floor_finish", "wall_finish", "roof_plane", "ceiling_zone"].includes(items[0].entity_type)) return null;
  return items.reduce((sum, item) => {
    const value = item.entity_type === "wall" || item.entity_type === "ceiling_zone" || item.entity_type === "roof_plane" ? number(item.data.net_area_m2) : number(item.data.area_m2);
    return sum + (value || 0);
  }, 0);
}
...
export function ReviewClassificationPage({ projectId }: { projectId: string }) {
  ...
  const query = useQuery({
    queryKey: ["review", projectId, floorId || null, "classification-summary", takeoffRevision, structureRevision, specialRevision],
