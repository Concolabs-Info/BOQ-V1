...
import { getScaleState, saveDrawingCalibration, saveFloorCalibration } from "../api";
...
export function ScalePage({ projectId }: { projectId: string }) {
  ...
  const scaleItems = useMemo(() => [...(state?.floors || []), ...(state?.drawing_sources || [])], [state?.drawing_sources, state?.floors]);

  useEffect(() => {
    if (!scaleItems.length) return;
    const saved = typeof window !== "undefined" ? window.sessionStorage.getItem(`autoboq:scale:floor:${projectId}`) : null;
    const next = scaleItems.find((item) => item.id === saved) ?? scaleItems.find((item) => item.status === "not_calibrated") ?? scaleItems[0];
    setSelectedFloorId((current) => current && scaleItems.some((item) => item.id === current) ? current : next.id);
  }, [projectId, scaleItems]);

  const floor = useMemo(() => scaleItems.find((item) => item.id === selectedFloorId) ?? null, [selectedFloorId, scaleItems]);
  ...
  async function save() {
    ...
    setError(null);
    try {
      const saveCalibration = floor.target_type === "drawing" ? saveDrawingCalibration : saveFloorCalibration;
      await saveCalibration(projectId, floor.id, {
        point_a: pointA,
        point_b: pointB,
        unit,
        ...(unit === "ft_in"
    ...
    <WorkflowStepPage projectId={projectId} stepKey="scale">
      <div className="grid h-[calc(100dvh-286px)] min-h-[720px] max-h-[1100px] grid-cols-[170px_minmax(0,1fr)_285px] overflow-hidden">
        <aside className="min-h-0 overflow-y-auto border-r border-slate-200 bg-white p-4">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Drawing scales</p>
          <div className="space-y-2">
            {scaleItems.map((item) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-slate-500">{item.inherited ? "Inherited from source drawing" : item.calibration ? `${item.calibration.mm_per_pixel.toFixed(4)} mm/px` : "Known-distance calibration"}</p>
              </button>
            ))}
          </div>
        </aside>
        ...
        <aside className="min-h-0 overflow-y-auto bg-white p-4">
              ...
              <div className="mt-3 space-y-2">
                {floor?.dimension_suggestions.slice(0, 5).map((suggestion) => (
                  <button key={suggestion.id} type="button" className="flex w-full items-center justify-between rounded-lg border border-blue-100 bg-white px-3 py-2 text-left text-sm hover:border-blue-300" onClick={() => {
                    ...
                    setDistanceValue(String(suggestion.value_mm)); setDistanceFeet(""); setDistanceInches(""); setUnit("mm"); setMode("calibration");
                  }}>
                    <span className="font-semibold text-slate-900">{suggestion.value_mm.toFixed(0)} mm</span>
          ...
          <Button className="mt-5 h-11 w-full" onClick={() => void save()} disabled={saving || !floor?.drawing_url || floor?.inherited}>{floor?.inherited ? "Scale inherited" : saving ? "Saving" : "Save calibration"}</Button>
        </aside>
      </div>
