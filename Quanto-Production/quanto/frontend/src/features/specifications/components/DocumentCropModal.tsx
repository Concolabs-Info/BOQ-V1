...
import { toOriginalPageRect } from "@/features/floor-plans/utils/coordinates";
import type { CropRect, DocumentOption, FloorOption, ScopeMode, SpecificationCategoryKey } from "../types";
...
export function DocumentCropModal({ category, documents, floors, defaultDrawingTypes, onClose, onSave }: Props) {
  const firstDocument = documents.find((item) => item.is_primary) || documents[0] || null;
  ...
  const [pageNumber, setPageNumber] = useState(selectedDocument?.pages[0]?.page_number || 1);
  ...
  async function save() {
    ...
    try {
      await onSave({
        ...
        scope_mode: scopeMode,
        floor_ids: floorIds,
        drawing_types: defaultDrawingTypes,
      });
      onClose();
    } finally {
      setSaving(false);
      ...
      <div className="flex h-[100dvh] w-screen flex-col overflow-hidden bg-white">
        <header className="flex shrink-0 flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-5 py-4">
          <div>
            ...
            <h3 className="mt-1 text-lg font-semibold text-slate-950">Crop from project PDF</h3>
