...
export function SourceCard({
  ...
  onRemove: () => void;
  onScopeChange: (scopeMode: ScopeMode, floorIds: string[], drawingTypes: string[]) => void;
}) {
  const inputRef = useRef<HTMLInputElement>(null);
  const previewUrl = useAssetUrl(source.preview_url);
  const [scopeSaving, setScopeSaving] = useState(false);

  async function changeScope(scopeMode: ScopeMode, floorIds: string[], mappedDrawingTypes = source.drawing_types) {
    setScopeSaving(true);
    try {
      await onScopeChange(scopeMode, floorIds, mappedDrawingTypes);
    } finally {
