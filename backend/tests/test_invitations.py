...
GUEST = CurrentUser(
    ...
    email="join@acme.com",
    full_name="Join",
    terms_accepted_at=datetime(2026, 9, 17, tzinfo=timezone.utc),
...
def test_email_already_in_a_company_ignores_soft_deleted_users(monkeypatch):
    seen = {}
    monkeypatch.setattr(
        invitations,
        "fetch_one",
        lambda sql, params=(): seen.update(sql=sql, params=params) or None,
    )
    assert invitations._email_already_in_a_company("join@acme.com") is False
...
def test_send_invites_inserts_hashed_token_and_calls_clerk(monkeypatch):
    ...
    monkeypatch.setattr(invitations, "_account_exists", lambda email: False)
    monkeypatch.setattr(invitations, "_pending_other_company", lambda email, company_id: False)
    ...
    assert any("expires_at <= now()" in sql for sql, _params in conn.calls)
...
def test_send_invites_keeps_local_invite_for_existing_account(monkeypatch):
    monkeypatch.setattr(invitations, "invitation_redirect_url", lambda: "http://localhost:3000/sign-up")
    monkeypatch.setattr(invitations, "_email_already_in_a_company", lambda email: False)
    monkeypatch.setattr(invitations, "_account_exists", lambda email: True)
    monkeypatch.setattr(invitations, "_pending_other_company", lambda email, company_id: False)
    monkeypatch.setattr(invitations, "owned_workspace_ids", lambda company_id, ids: [])
    seen = {"clerk": False}
    ...
    def boom(*args, **kwargs):
        seen["clerk"] = True
        raise AssertionError("Clerk invitation must not run for an existing account")

    monkeypatch.setattr(invitations, "create_invitation", boom)
    conn = FakeConn(insert_id="inv-existing")
    patch_tx(monkeypatch, conn)
    result = invitations.send_invites(ADMIN, MEMBERSHIP, [{"email": "join@acme.com", "role": "qs"}])
    assert result.sent == 1
    assert result.existing_accounts == ["join@acme.com"]
    assert seen["clerk"] is False
    assert any("INSERT INTO invitation" in sql for sql, _params in conn.calls)


def test_send_invites_treats_clerk_existing_user_as_sent(monkeypatch):
    monkeypatch.setattr(invitations, "invitation_redirect_url", lambda: "http://localhost:3000/sign-up")
    monkeypatch.setattr(invitations, "_email_already_in_a_company", lambda email: False)
    monkeypatch.setattr(invitations, "_account_exists", lambda email: False)
    monkeypatch.setattr(invitations, "_pending_other_company", lambda email, company_id: False)
    ...
    monkeypatch.setattr(
        invitations,
        "create_invitation",
        lambda *a, **k: (_ for _ in ()).throw(ClerkApiError("email address is already associated with an account")),
    )
    patch_tx(monkeypatch, FakeConn())
    result = invitations.send_invites(ADMIN, MEMBERSHIP, [{"email": "join@acme.com", "role": "qs"}])
    assert result.sent == 1
...
def test_send_invites_rejects_pending_invite_at_another_company(monkeypatch):
    monkeypatch.setattr(invitations, "invitation_redirect_url", lambda: "http://localhost:3000/sign-up")
    monkeypatch.setattr(invitations, "_email_already_in_a_company", lambda email: False)
    monkeypatch.setattr(invitations, "_pending_other_company", lambda email, company_id: True)
    result = invitations.send_invites(ADMIN, MEMBERSHIP, [{"email": "join@acme.com", "role": "qs"}])
    assert result.sent == 0
    assert "another company" in result.failures[0].reason


def test_send_invites_revokes_expired_pending_from_any_company(monkeypatch):
    monkeypatch.setattr(invitations, "invitation_redirect_url", lambda: "http://localhost:3000/sign-up")
    monkeypatch.setattr(invitations, "_email_already_in_a_company", lambda email: False)
    monkeypatch.setattr(invitations, "_account_exists", lambda email: True)
    monkeypatch.setattr(invitations, "_pending_other_company", lambda email, company_id: False)
    monkeypatch.setattr(invitations, "owned_workspace_ids", lambda company_id, ids: [])
    conn = FakeConn(insert_id="inv-expired")
    patch_tx(monkeypatch, conn)
    result = invitations.send_invites(ADMIN, MEMBERSHIP, [{"email": "join@acme.com", "role": "qs"}])
    assert result.sent == 1
    revoke = next(sql for sql, _params in conn.calls if "UPDATE invitation SET status = 'revoked'" in sql)
    assert "expires_at <= now()" in revoke
...
def test_decline_pending_invites_revokes_open_rows(monkeypatch):
    monkeypatch.setattr(invitations, "get_company_membership", lambda user_id: None)
    monkeypatch.setattr(
        invitations,
        "fetch_all",
        lambda sql, params=(): [{"id": "inv-1", "email": GUEST.email, "clerk_invitation_id": "clerk_1", "status": "pending"}],
    )
    seen = {"clerk": None, "sql": None}
    monkeypatch.setattr(invitations, "_revoke_clerk_invite", lambda row: seen.update(clerk=row["id"]))
    monkeypatch.setattr(invitations, "execute", lambda sql, params=(): seen.update(sql=sql, params=params))
    assert invitations.decline_pending_invites(GUEST) == 1
    assert seen["clerk"] == "inv-1"
    assert "revoked" in seen["sql"]


def test_decline_pending_invites_noop_when_already_a_member(monkeypatch):
    monkeypatch.setattr(invitations, "get_company_membership", lambda user_id: MEMBERSHIP)
    assert invitations.decline_pending_invites(ADMIN) == 0


def test_list_pending_invitations_marks_existing_accounts(monkeypatch):
    monkeypatch.setattr(
        invitations,
        "fetch_all",
        lambda sql, params=(): [
    ...
    monkeypatch.setattr(invitations, "_account_exists", lambda email: True)
    rows = invitations.list_pending_invitations("c1")
    assert rows[0]["existing_account"] is True
