import { isDevInstance } from "../clerk-errors";

function ClerkLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 26 32" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden className={className}>
      <path
        d="M16.0003 21C18.7617 21 21.0003 18.7614 21.0003 16C21.0003 13.2386 18.7617 11 16.0003 11C13.2389 11 11.0003 13.2386 11.0003 16C11.0003 18.7614 13.2389 21 16.0003 21Z"
        fill="#9394A1"
      />
      <path
...
export function SecuredByClerk() {
  return (
    <div className="flex flex-col items-center gap-1.5 pt-2">
      <a
        href="https://clerk.com?utm_source=quanto&utm_medium=auth_footer"
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center gap-1.5 text-[0.8rem] text-slate-500 transition-opacity hover:opacity-80"
      >
        Secured by
        <span className="inline-flex items-center gap-1">
          <ClerkLogo className="h-3.5 w-auto" />
          <span className="font-semibold tracking-tight text-[#9394A1]">clerk</span>
        </span>
      </a>
      {isDevInstance ? <p className="text-[12px] font-medium text-[#F36B16]">Development mode</p> : null}
    </div>
  );
}
