...
export function RoomInspector({
  projectId, room, rooms, saving, revisions, onSave, onEdit, onConfirm, onExclude, onDelete,
  onSplitLine, onAddCutout, onMerge, onRestore, onRestoreRevision, onDeleteCutout,
  ...
  useEffect(() => {
    setDraft(room ? {
      name: room.name, room_type: room.room_type, review_status: room.status,
      ...
      {!room.is_finish_zone ? (
        <div className="rounded-xl border border-slate-200 bg-white p-4 text-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="font-semibold">Visible floor finish</p>
              <p className="mt-1 text-xs text-slate-500">Managed from the canonical Floor finishes assignment.</p>
            </div>
            <Link className="shrink-0 text-xs font-semibold text-blue-600" href={`/workspace/${projectId}/floors?tab=finishes`}>Open finishes</Link>
          </div>
          <div className="mt-3 flex justify-between gap-3 border-t border-slate-100 pt-3">
            <span className="text-slate-500">Assigned finish</span>
            <strong className="text-right">{room.finish_summary?.name || "Not assigned"}</strong>
          </div>
          {room.finish_summary ? <div className="mt-2 flex justify-between gap-3"><span className="text-slate-500">Status</span><strong className="capitalize">{room.finish_summary.status.replaceAll("_", " ")}</strong></div> : null}
        </div>
      ) : null}
