...
export function ModelReviewPage({ projectId }: { projectId: string }) {
  ...
  async function patchElement(element: ReviewElement, patch: Record<string, unknown>) {
    ...
    const result = await editQueue.enqueue({
      queryKey: queryKey(projectId, floorId),
      mutation: () => updateReviewElement(projectId, element.floor_id, element.id, patch),
      optimistic: (current: ModelReviewState) => ({...current,elements:current.elements.map((item)=>item.id===element.id?{...item,...patch}:item)}),
      invalidations: [
        {queryKey:["model-review",projectId],refetchType:"active"},
        {queryKey:["walls",projectId],refetchType:"none"},
        {queryKey:["floors",projectId],refetchType:"none"},
      ...
      errorMessage:"The element could not be saved.",onError:setError,
    });
    if (result && typeof result === "object" && "record" in result) updateElementCache(element.id,(result as {record:ReviewElement}).record);
  }
          ...
          <ElementInspector
            ...
            onProperty={async (name, value, unit) => {
              ...
              await editQueue.enqueue({
                queryKey:queryKey(projectId,floorId),mutation:()=>updateReviewProperty(projectId,element.floor_id,element.id,name,value,unit),
                optimistic:(current:ModelReviewState)=>({...current,elements:current.elements.map((item)=>item.id===element.id?{...item,resolved_data:{...item.resolved_data,[name]:value},properties:item.properties.map((property)=>property.property_name===name?{...property,value,unit:unit??null,source:"user_confirmed",is_confirmed:true}:property)}:item)}),
                invalidations:[
                  {queryKey:["model-review",projectId],refetchType:"active"},{queryKey:["walls",projectId],refetchType:"none"},
                  {queryKey:["review",projectId],refetchType:"none"},{queryKey:["boq",projectId],refetchType:"none"},
                ],errorMessage:"The property could not be saved.",onError:setError,
              });
            }}
            onSchedule={async (entryId) => {
              ...
              const element=selected;
              await editQueue.enqueue({
                queryKey:queryKey(projectId,floorId),mutation:()=>assignScheduleEntry(projectId,element.floor_id,element.id,entryId),
                optimistic:(current:ModelReviewState)=>({...current,elements:current.elements.map((item)=>item.id===element.id?{...item,assigned_schedule_entry_id:entryId,status:"confirmed" as const}:item)}),
                invalidations:[
                  {queryKey:["model-review",projectId],refetchType:"active"},{queryKey:["walls",projectId],refetchType:"none"},
                  {queryKey:["review",projectId],refetchType:"none"},{queryKey:["boq",projectId],refetchType:"none"},
                ],errorMessage:"The schedule could not be assigned.",onError:setError,
              });
            }}
          />
