...
import type { Centerline, WallPatch, WallsState } from "../types";
import { WallInspector } from "./WallInspector";
import { WallOverlay } from "./WallOverlay";
import { WallFinishesWorkspace } from "./WallFinishesWorkspace";
...
export function WallsPage({ projectId }: { projectId: string }) {
  ...
  const [activeTab, setActiveTab] = useState<"walls" | "finishes">("walls");
  const editQueue = useOptimisticMutationQueue();

  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("tab") === "finishes") setActiveTab("finishes");
  }, []);
  ...
  async function act(action: () => Promise<unknown>, optimistic?: (current: WallsState) => WallsState) {
    if (optimistic) {
      return editQueue.enqueue<WallsState>({
        ...
        errorMessage: "The wall operation could not be completed.", onError: setError,
      });
    }
    setSaving(true);
    setError(null);
  ...
  async function save(patch: WallPatch) {
    ...
    const changes = centerline ? { ...patch, centerline } : patch;
    const completed = await act(() => updateWall(projectId, floorId, wallId, changes), (current) => ({
      ...current,
      walls: current.walls.map((wall) => wall.id === wallId ? {
        ...wall, ...changes,
        height_override_mm: changes.use_floor_height ? null : changes.height_override_mm ?? wall.height_override_mm,
        height_source: changes.use_floor_height ? "floor" : changes.height_override_mm != null ? "wall" : wall.height_source,
        status: changes.review_status || "confirmed", user_confirmed: true,
      } : wall),
    }));
    if (completed) {
  ...
  return (
    <WorkflowStepPage projectId={projectId} stepKey="walls">
      <div className="flex h-14 items-end gap-1 border-b border-slate-200 bg-white px-6">
        <button type="button" onClick={() => setActiveTab("walls")} className={activeTab === "walls" ? "h-11 border-b-2 border-blue-600 px-4 text-sm font-semibold text-blue-700" : "h-11 px-4 text-sm font-semibold text-slate-500 hover:text-slate-900"}>Walls</button>
        <button type="button" onClick={() => setActiveTab("finishes")} className={activeTab === "finishes" ? "h-11 border-b-2 border-blue-600 px-4 text-sm font-semibold text-blue-700" : "h-11 px-4 text-sm font-semibold text-slate-500 hover:text-slate-900"}>Wall finishes</button>
      </div>
      {activeTab === "walls" ? <>
              ...
              {walls.map((wall) => {
                ...
                return (
                  <button
                    ...
                    className={wall.id === selectedId ? "lazy-list-item w-full rounded-lg border border-blue-300 bg-blue-50 px-3 py-2 text-left" : "lazy-list-item w-full rounded-lg border border-transparent px-3 py-2 text-left hover:border-slate-200 hover:bg-slate-50"}
                  >
        ...
        <aside className="min-h-0 overflow-y-auto bg-white">
          ...
          <WallInspector
            ...
            onDelete={async () => {
              ...
              await act(() => deleteWall(projectId, floorId, wallId), (current) => ({ ...current, walls: current.walls.filter((wall) => wall.id !== wallId) }));
            }}
          />
      ...
      </> : <WallFinishesWorkspace projectId={projectId} />}
    </WorkflowStepPage>
  );
}
